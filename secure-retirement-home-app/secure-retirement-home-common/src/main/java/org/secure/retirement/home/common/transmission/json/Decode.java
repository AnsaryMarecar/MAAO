...
public class Decode {
	...
	public static void main(String[] args) {
		...
		/**
		 * try { to_decode("","Send_information"); } catch (IOException e) {
		 * e.printStackTrace(); }
		 **/
	}
	...
	public static Object[] to_decode(String param_todecode, String param_class) throws IOException {
		...
		if (param_todecode != null) {
			...
			case "Sensor":
				...
				// objectMapper.configure(DeserializationFeature.EAGER_DESERIALIZER_FETCH,
				// false);
				break;
			case "Sensors":
				to_decodeSensor(param_todecode);
	...
	public static Object[] to_decodeHistoric(String param_todecode) throws IOException {
		...
		for (int i = 0; i < parts.length; i = i + 2) {
			Historic historic = new Historic(new Sensor(Integer.parseInt(parts[i])), Double.parseDouble(parts[i + 1]));
